"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CostCenterController = void 0;
const common_1 = require("@nestjs/common");
const cost_center_service_1 = require("./cost-center.service");
const connection_service_1 = require("../connection.service");
let CostCenterController = class CostCenterController {
    constructor(_costCenterService, _connService) {
        this._costCenterService = _costCenterService;
        this._connService = _connService;
    }
    async listAll() {
        return this._costCenterService.readAll();
    }
};
__decorate([
    common_1.Get('listAll'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", Promise)
], CostCenterController.prototype, "listAll", null);
CostCenterController = __decorate([
    common_1.Controller('cost-center'),
    __metadata("design:paramtypes", [cost_center_service_1.CostCenterService,
        connection_service_1.ConnectionService])
], CostCenterController);
exports.CostCenterController = CostCenterController;
//# sourceMappingURL=cost-center.controller.js.map